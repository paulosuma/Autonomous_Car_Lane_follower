...
class Maze(object):
    ...
    def sensor_model(self, coordinates, sensor_limit, orientation = 0):
        ...
        pos_y = y
        d5 = 0
        dx = np.cos(orientation+np.pi/4) * 1 - np.sin(orientation+np.pi/4) * 1
        dy = np.sin(orientation+np.pi/4) * 1 + np.cos(orientation+np.pi/4) * 1
        ...
        pos_y = y
        d7 = 0
        dx = np.cos(orientation-np.pi*(3/2)) * 1 - np.sin(orientation-np.pi*(3/2)) * 1
        dy = np.sin(orientation-np.pi*(3/2)) * 1 + np.cos(orientation-np.pi*(3/2)) * 1
